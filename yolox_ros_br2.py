...
class yolox_ros(Node):
    def __init__(self) -> None:
        ...
        self.pub_bbox = self.create_publisher(Detection2D,"detection", 100)
        ...
        #self.sub = self.create_subscription(Image,"depth_camera/image_raw",self.imageflow_callback, qos_profile=qos_policy)
        self.sub = self.create_subscription(Image, "/head_front_camera/rgb/image_raw", self.imageflow_callback, 10)
    ...
    def setting_yolox_exp(self) -> None:
        ...
        WEIGHTS_PATH = '/home/mani/ros2_ws/yolox/src/yolox_ros/weights/yolox_s.pth'
        ...
        self.predictor = PredictorTiago(model, exp, COCO_CLASSES, trt_file, decoder, device, fp16, legacy)
    ...
    def imageflow_callback(self,msg:Image) -> None:
        try:
            ...
            try:
                ...
                xcen = (xmin+xmax)/2.0
                ycen = (ymin+ymax)/2.0
                xsize = abs(abs(xmin)- abs(xmax))
                ysize = abs(abs(ymin)- abs(ymax))
                detection_bbox = Detection2D()
                detection_bbox.header = msg.header
                detection_bbox.bbox.center.x = xcen
                detection_bbox.bbox.center.y = ycen
                detection_bbox.bbox.size_x = xsize * 1.0
                detection_bbox.bbox.size_y = ysize * 1.0
                self.pub_bbox.publish(detection_bbox)
                ...
                #if (self.imshow_isshow):
                 #   logger.info(cls_names+ " detected")
